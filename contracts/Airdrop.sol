...
contract Airdrop is Ownable,Pausable{
    ...
    function sendTokens(address[] memory _addr,uint[] memory _amt) public onlyOwner {
        require(_addr.length == _amt.length,"Incorrect params");
        for (uint8 i = 0;i<_addr.length;i++) {
        token.transfer(_addr[i],_amt[i]);
        }
    }
    ...
    function failSafe(IBEP20 _token,address to,uint amount) public onlyOwner {
